...
import { uploadImagesAndRegister, getSignedDownloadUrls, createZipBundleSignedUrl, sendDeliveryLinks, } from '../../domain/media/services/image-upload-service.js';
...
deliveryRoutes.post('/images', async (c) => {
    ...
    if (contentType.includes('multipart/form-data')) {
        ...
        mode = form.mode || 'links';
        expiresIn = form.expiresIn ? Number(form.expiresIn) : 3600;
        if (form.delivery) {
            try {
                delivery = JSON.parse(form.delivery);
            }
            catch {
                delivery = undefined;
            }
    ...
    let response = { linkId, uploaded, errors };
    if (mode === 'zip') {
...
deliveryRoutes.get('/images/:linkId', async (c) => {
    ...
    const mode = (c.req.query('mode') || 'links');
